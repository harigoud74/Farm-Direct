      ...
      <div className="bg-white w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden border border-stone-200 animate-in fade-in zoom-in-95 duration-200 max-h-[92vh] flex flex-col">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-stone-500">
                      Farmer: {product.farmerName} &bull; {farmer?.experienceYears || 15} yrs farming
                    </p>
                    <div className="flex items-center gap-2 text-[11px] text-stone-600 mt-0.5">
                      ...
                      <span>&bull;</span>
                      <span>{farmer?.totalOrdersFulfilled || 300}+ orders fulfilled</span>
                    </div>
                  </div>
                </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 text-xs bg-stone-50 border border-stone-200 rounded-xl p-3.5">
                <div>
                  <span className="text-[11px] text-stone-400 block">Quality Grade</span>
                  <span className="font-bold text-stone-800">{product.qualityGrade}</span>
                </div>
                <div>
                  <span className="text-[11px] text-stone-400 block">Farming Method</span>
                  <span className="font-bold text-emerald-800">{product.farmingMethod}</span>
                </div>
                <div>
                  <span className="text-[11px] text-stone-400 block">Harvest Timing</span>
                  <span className="font-bold text-stone-800">{product.harvestDate}</span>
                </div>
                <div>
                  <span className="text-[11px] text-stone-400 block">Natural Shelf Life</span>
              ...
              <div className="p-4 bg-emerald-50/70 border border-emerald-200 rounded-2xl space-y-3">
                <div className="flex items-baseline justify-between">
                  <div>
                    <span className="text-xs text-emerald-900 block font-medium">Farmer Direct Rate</span>
                  ...
                  <div className="text-right">
                    <span className="text-xs text-stone-500 block">In Stock</span>
                    <span className="text-sm font-bold text-emerald-800">
                      {product.availableQuantityKg} {product.unit} available
                    </span>
                  </div>
                </div>
        ...
        <div className="px-6 py-4 border-t border-stone-200 bg-stone-50 flex items-center justify-between gap-4">
          <div className="text-xs text-stone-500">
            Min order: {product.minOrderQuantityKg} {product.unit} &bull; Cold-chain delivery included
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={onClose}
            ...
            >
              ...
              <span>Add to Cart ({formatINR(product.pricePerKg * quantity)})</span>
            </button>
          </div>
        </div>
      </div>
